...
export default function Edit({ classes = [], trainers = [], appointment, appointmentSessions = [], weekdayOptions = [], updateScopeOptions = [] }) {
                ...
                <form onSubmit={submit} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 md:p-8">
                    <div className="grid gap-8 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
                        ...
                        <div className="space-y-4 rounded-3xl border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-950/40">
                            ...
                            <label className="inline-flex items-start gap-3 rounded-2xl border border-slate-200 p-4 text-sm bg-white dark:border-slate-700 dark:bg-slate-900">
    ...
    {weekdayOptions.map((day) => {
        ...
        return (
            <div key={day.value} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all dark:border-slate-800 dark:bg-slate-900">
                {/* Header: Checkbox + Nama Hari */}
                <div className="flex items-center gap-3">
                    <input
                        ...
                        className="h-5 w-5 cursor-pointer rounded border-slate-300 text-primary-600 focus:ring-primary-500"
                    />
                    <label 
                        htmlFor={`check-${day.value}`} 
                        className="cursor-pointer select-none text-base font-semibold text-slate-800 dark:text-slate-100"
                    >
                        {weekdayLabels[day.value] || day.label}
                    </label>
                </div>

                {/* Slot Area: Hanya muncul jika checkbox aktif */}
                {daySchedule.active && (
                    <div className="mt-5 space-y-6 border-t border-slate-100 pt-5 dark:border-slate-800">
                        {daySchedule.slots.map((slot, slotIndex) => (
                            <div key={`${day.value}-${slotIndex}`} className="group relative">
                                <div className="grid grid-cols-1 gap-4 items-end sm:grid-cols-2 lg:grid-cols-[1fr_auto_1fr_auto]">
                                    
                                    {/* Kolom Jam Mulai */}
                                    <div className="space-y-1.5">
                                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Mulai</span>
                                        <TimeSelect
                                    ...
                                    {/* Pemisah Titik Dua (Desktop Only) */}
                                    <div className="hidden pb-3 text-xl font-bold text-slate-300 lg:block">:</div>

                                    {/* Kolom Jam Selesai */}
                                    <div className="space-y-1.5">
                                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Selesai</span>
                                        <TimeSelect
                                            hour={slot.end_hour}
                                    ...
                                    <div className="flex justify-end pb-1">
                                        <button
                                            ...
                                            className="inline-flex items-center gap-1.5 rounded-lg border border-rose-100 px-3 py-2 text-xs font-medium text-rose-500 transition hover:bg-rose-50 hover:border-rose-200"
                                        >
                                            <IconTrash size={14} />
                                            <span>Hapus</span>
                                        </button>
                                ...
                                <div className="mt-2 space-y-1">
                                    {(getSlotError(errors, day.value, slotIndex, "time_range") || 
                                      getSlotError(errors, day.value, slotIndex, "start_hour")) && (
                                        <p className="text-xs text-rose-500">
                                            {getSlotError(errors, day.value, slotIndex, "time_range") || "Periksa kembali input jam."}
                                        </p>
                                    )}
                                </div>
                            </div>
