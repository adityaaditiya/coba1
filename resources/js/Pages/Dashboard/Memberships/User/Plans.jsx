...
export default function Plans({ plans }) {
    // Menyimpan id paket yang sedang di-expand (contoh format: { 1: true, 2: false })
    const [expandedPlans, setExpandedPlans] = useState({});

    const toggleClasses = (planId) => {
        ...
        <>
            ...
            <div className="py-6">
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {plans.map((plan) => {
                        // Cek apakah paket ini statusnya sedang dibuka atau tidak
                        const isExpanded = !!expandedPlans[plan.id];
                        // Tentukan kelas mana saja yang mau ditampilkan (maksimal 5 jika tidak dibuka)
                        const visibleClasses = isExpanded ? plan.classes : plan.classes.slice(0, 5);
                                            ...
                                            {plan.classes && plan.classes.length > 0 ? (
                                                <>
                                                    ...
                                                    {plan.classes.length > 5 && (
                                                        <button
                                                            type="button"
                                                            onClick={() => toggleClasses(plan.id)}
                                                            className="inline-flex items-center gap-1 rounded-lg bg-slate-200 px-3 py-1.5 text-xs font-bold text-slate-800 hover:bg-slate-300 transition-colors dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600"
                                                        >
                                                            {isExpanded ? "Lihat lebih sedikit" : `+ ${plan.classes.length - 5} Kelas Lainnya`}
                                                        </button>
                                                    )}
                                                </>
                                            ) : (
                                                <span className="text-xs text-slate-400 italic">Tidak ada akses kelas</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
