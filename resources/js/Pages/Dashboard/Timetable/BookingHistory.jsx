...
export default function BookingHistory({ bookings, filters = {} }) {
    ...
    const applyFilters = (event) => {
        ...
        router.get(route("bookings.history"), filterData, {
            preserveScroll: true,
            preserveState: true,
        });
    ...
    const resetFilters = () => {
        ...
        router.get(route("bookings.history"), defaultFilters, {
            preserveScroll: true,
            preserveState: true,
            replace: true,
        });
    ...
    const handleReschedule = (booking) => {
        ...
        const comboboxItems = targets.map((target) => ({
            id: target.id,
            label: `${target.schedule_at} • ${target.class_name || "-"} • sisa ${target.remaining_slots} slot`,
            searchKey: `${target.schedule_at} ${target.class_name || ""}`.toLowerCase(),
        }));
            ...
            html: `
                <div class="space-y-5 px-1">
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 text-left shadow-sm">
                        ...
                        <p class="text-sm text-slate-600">${currentSchedule}</p>
                    </div>
                    ...
                    <div class="text-left">
                        ...
                        <div class="relative">
                            ...
                            <input 
                                ...
                                type="text" 
                                placeholder="Ketik tanggal atau nama kelas..." 
                                    ...
                                    rows.map((booking, index) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-4 text-center text-sm">
                                                <span className="inline-flex items-center gap-1 rounded-lg bg-primary-50 px-2.5 py-1 text-primary-700">
                                                    <IconUsers size={14} />
                                            ...
                                            <td className="px-4 py-4 text-center">
                                                <span
                                                    className={`rounded-md px-2.5 py-1 text-xs font-semibold capitalize ${statusClass[booking.status] || "bg-slate-100 text-slate-700"}`}
                                                >
                                                    {booking.status || "-"}
                                                </span>
                                            </td>
...
BookingHistory.layout = (page) => <DashboardLayout children={page} />;
