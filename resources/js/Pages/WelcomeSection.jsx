    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-gradient-to-b from-wellness-beige to-white text-wellness-text">
                ...
                {pageKey === "trainers" && (
                    <>
                        <section className="grid max-w-6xl gap-8 px-4 mx-auto pb-16 md:grid-cols-2 lg:grid-cols-3">
                            {trainers.length === 0 && (
                            ...
                            {trainers.map((trainer) => (
                                <article key={trainer.id} className="flex flex-col overflow-hidden transition-all duration-300 bg-white border border-slate-100 shadow-sm rounded-3xl hover:shadow-xl hover:-translate-y-1 hover:border-primary-100 group">
                                    
                                    {/* Bagian Image */}
                                    {trainer.photo && (
                                        <div className="relative overflow-hidden h-64">
                                            <img 
                                                src={imageUrl("trainers", trainer.photo)} 
                                                alt={trainer.name} 
                                                className="object-cover w-full h-full transition-transform duration-700 group-hover:scale-105"
                                                onError={(e) => {
                                                    e.target.onerror = null; 
                                                    e.target.src = "https://ui-avatars.com/api/?name=" + trainer.name + "&background=F8FAFC&color=334155&size=512";
                                                }}
                                            />
                                            {/* Gradient Overlay untuk transisi halus dari gambar ke teks */}
                                    ...
                                    <div className="flex flex-col flex-1 p-6 space-y-4">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2 mb-2">
                                                <span className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold rounded-full bg-primary-50 text-primary-700">
                                                    <IconYoga size={14} /> {trainer.expertise || "Spesialisasi Kosong"}
                                                </span>
                                            </div>
                                            <h3 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                                                <IconUser size={18} className="text-slate-400" /> {trainer.name}
                                            </h3>
                                        </div>

                                        {/* Biodata Singkat (Dipotong max 2 baris) */}
                                        ...
                                        <button 
                                            ...
                                            className="inline-flex items-center justify-center w-full px-4 py-2.5 text-sm font-semibold transition-colors border rounded-xl text-primary-600 border-primary-200 bg-primary-50/50 hover:bg-primary-100 hover:border-primary-300 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-1"
                                        >
                        ...
                        {selectedTrainer && (
                            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
                                {/* Backdrop Hitam Transparan & Blur */}
                                <div 
                                    className="absolute inset-0 transition-opacity bg-slate-900/40 backdrop-blur-sm"
                                    onClick={() => setSelectedTrainer(null)} // Tutup jika area luar diklik
                                ></div>

                                {/* Kotak Modal Utama */}
                                <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white shadow-2xl rounded-3xl ring-1 ring-slate-900/5 animate-in fade-in zoom-in-95 duration-200 custom-scrollbar">
                                    
                                    {/* Tombol Close (X) */}
                                    <button 
                                        onClick={() => setSelectedTrainer(null)}
                                        className="absolute flex items-center justify-center w-8 h-8 transition-colors bg-white border border-slate-200 rounded-full top-4 right-4 text-slate-400 hover:text-slate-600 hover:bg-slate-50 shadow-sm"
                                    >
                                    ...
                                    <div className="p-6 sm:p-8 space-y-8">
                                        ...
                                        <div className="flex items-start gap-5">
                                            {selectedTrainer.photo ? (
                                                <img 
                                                    src={imageUrl("trainers", selectedTrainer.photo)} 
                                                    alt={selectedTrainer.name} 
                                                    className="object-cover w-20 h-20 border-2 border-white shadow-md rounded-2xl bg-slate-100"
                                                    onError={(e) => { e.target.src = "https://ui-avatars.com/api/?name=" + selectedTrainer.name; }}
                                                />
                                            ) : (
                                                <div className="flex items-center justify-center w-20 h-20 rounded-2xl bg-slate-100 text-slate-400">
                                                    <IconUser size={32} />
                                                </div>
                                            )}
                                            <div>
                                                <h2 className="text-2xl font-bold text-slate-900">{selectedTrainer.name}</h2>
                                                <p className="inline-flex items-center gap-1.5 mt-2 px-3 py-1 text-sm font-medium rounded-full bg-primary-50 text-primary-700">
                                                    <IconYoga size={16} /> {selectedTrainer.expertise || "Spesialisasi Kosong"}
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-6">
                                            <div>
                                                <h4 className="text-sm font-bold tracking-wider text-slate-900 uppercase">Keahlian</h4>
                                                <p className="mt-2 text-sm leading-relaxed text-slate-600 whitespace-pre-line ">
                                                    {selectedTrainer.biodata || "Biodata trainer belum diisi."}
                                                </p>
                                            </div>

                                            <div className="grid gap-6 pt-6 border-t border-slate-100 sm:grid-cols-2">
                                                <div className="p-5 border bg-slate-50/50 border-slate-100 rounded-2xl">
                                                    <h4 className="flex items-center gap-2 text-sm font-bold text-slate-900">
                                                        <svg className="w-5 h-5 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z"></path></svg>
                                                        Workshop
                                                    </h4>
                                                    <p className="mt-3 text-sm leading-relaxed text-slate-600 whitespace-pre-line">
                                                        {selectedTrainer.workshop || "Belum ada riwayat workshop."}
                                                    </p>
                                                </div>
                                                ...
                                                <div className="p-5 border bg-slate-50/50 border-slate-100 rounded-2xl">
                                                    ...
                                                    <p className="mt-3 text-sm leading-relaxed text-slate-600 whitespace-pre-line">
                                                        {selectedTrainer.courses || "Belum ada riwayat courses."}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
